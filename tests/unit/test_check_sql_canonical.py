...
class TestCheckSqlCanonical:
    def test_default_paths_document_contract(self) -> None:
        assert CANONICAL == Path("docs/04_Database_Schemas.sql")
        assert DEPLOYABLE == Path("infra/postgres/init/001_schema.sql")
    ...
    def test_newline_drift_returns_one(self, tmp_path: Path) -> None:
        with patch("scripts.check_sql_canonical.CANONICAL", tmp_path / "canonical.sql"), \
             patch("scripts.check_sql_canonical.DEPLOYABLE", tmp_path / "deployable.sql"):
            (tmp_path / "canonical.sql").write_bytes(b"-- schema\n")
            (tmp_path / "deployable.sql").write_bytes(b"-- schema\r\n")
            assert main() == 1
...
class TestCliInvocation:
    def test_runs_as_script(self) -> None:
        result = subprocess.run(
            [sys.executable, "scripts/check_sql_canonical.py"],
            capture_output=True,
            text=True,
        )

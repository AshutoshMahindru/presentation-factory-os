...
def test_outbox_worker_is_idempotent_for_project_node_merge():
    ...
    first = subprocess.run(["python", "-m", "jobs.outbox_worker"], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    second = subprocess.run(["python", "-m", "jobs.outbox_worker"], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
